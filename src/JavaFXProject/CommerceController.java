...
public class CommerceController {
	...
	@FXML
	private TableColumn<Vente, String> colVenteRef, colVenteVehicule, colVenteClient, colVenteDate;
	@FXML
	private TableColumn<Vente, Double> colVenteMontant;
	@FXML
	private TableView<Client> tableClients;
	@FXML
	private TableColumn<Client, String> colClientNom, colClientEmail, colClientVip;

	@FXML
	private Button btnNouvelleVente, btnNouveauClient, btnFactures;
	@FXML
	...
	public void initialize() {
		...
		if (!Session.isAdmin()) {
			if (btnNouvelleVente != null)
				btnNouvelleVente.setVisible(false);
			if (btnNouveauClient != null)
				btnNouveauClient.setVisible(false);
			if (btnFactures != null) {
	...
	private void chargerDonnees() {
		...
		colVenteVehicule.setCellValueFactory(c -> new SimpleStringProperty(
				c.getValue().getVehicule() != null ? c.getValue().getVehicule().getModele() : "?"));
		colVenteClient.setCellValueFactory(c -> new SimpleStringProperty(
				c.getValue().getClient() != null ? c.getValue().getClient().getNom() : "?"));
		...
		colClientNom.setCellValueFactory(
				c -> new SimpleStringProperty(c.getValue().getNom() + " " + c.getValue().getPrenom()));
	...
	private void handleNouvelleVente() {
		openPopup("FormulaireVente.fxml");
	}

	@FXML
	private void handleNouveauClient() {
		openPopup("FormulaireClient.fxml");
	}

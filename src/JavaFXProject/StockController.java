...
public class StockController {
	...
	@FXML
	private TableView<Vehicule> tableStock;
	@FXML
	private TableColumn<Vehicule, Integer> colId;
	@FXML
	private TableColumn<Vehicule, String> colMarque, colModele, colStatut;
	@FXML
	private TableColumn<Vehicule, Double> colPrix;

	@FXML
	private Button btnAjouterVehicule, btnFactures;
	@FXML
	...
	public void initialize() {
		try {
			...
			if (!Session.isAdmin()) {
				if (btnAjouterVehicule != null)
					btnAjouterVehicule.setVisible(false);
				if (btnFactures != null) {
		...
		colStatut.setCellFactory(column -> new TableCell<Vehicule, String>() {
			...
			protected void updateItem(String item, boolean empty) {
				...
				if (empty || item == null) {
					...
					badge.setStyle(
							"-fx-font-weight: bold; -fx-padding: 3 10; -fx-background-radius: 5; -fx-text-fill: white;");
					if ("VENDU".equals(item))
						...
						badge.setStyle(badge.getStyle() + "-fx-background-color: #ff9800;");
					setGraphic(badge);
					setText(null);
				}
			}
	...
	private void configurerFiltres() {
		...
		List<String> marques = masterData.stream().map(Vehicule::getMarque).distinct().sorted()
